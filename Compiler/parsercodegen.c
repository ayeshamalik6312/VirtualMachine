...
void statement(Symbol *t, int *i, int size);
...
void statement(Symbol *t, int *i, int size) {
  if (t[*i].tokenVal == identsym) {
    ...
    (*i)++;
    // Here add logic for EXPRESSION and emit STO
    // emit STO (M = table[symIdx].addr)
  }

  // Handling 'begin' and 'end'
  else if (t[*i].tokenVal == beginsym) {
    int nested = 1;
    while ((*i)++ < size && nested > 0) {
      if (t[*i].tokenVal == beginsym)
        nested++;
      else if (t[*i].tokenVal == endsym)
        nested--;
    }
    if (nested != 0) {
      printf("Error: 'begin' must be followed by 'end'\n");
  ...
  // Add more conditions as necessary for other statement types
}
void condition(Symbol *t) {}
void expression(Symbol *t) {}
void term(Symbol *t) {}
